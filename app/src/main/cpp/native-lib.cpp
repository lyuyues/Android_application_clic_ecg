...
int gqrsMain(int argc, char **argv)
{
    char *p;
    ...
    *(--p) = '\0';
    ...
    annot.aux = (unsigned char*)auxbuf;
    annot.anntyp = NOTE;
...
void *gcalloc(size_t nmemb, size_t size)
{
    ...
    if ((p == NULL) ||
        ((gcn >= gcnmax) &&
         (q = (void **) realloc(gclist, (gcnmax += 32)*(sizeof(void *)))) == NULL)) {
...
void init()
{
    memset(auxbuf, 0, 1+255+1);		/* 'aux' string buffer for annotations */
    pname = NULL;			/* name of this program, used in messages */
    record = NULL;		/* name of input record */
    config = NULL;		/* configuration file, if any */
    countdown = -1;		/* if > 0, ticks remaining (see gqrs()) */
    debug = 0;			/* if non-zero, generate debugging output */
    minutes = 0;		/* minutes elapsed in the current hour */
    nsig = 0;			/* number of signals */
    pthr = 0;			/* peak-detection threshold */
    qthr = 0;			/* QRS-detection threshold */
    pthmin = 0;
    qthmin = 0;		/* minimum values for pthr and qthr */
    rrdev = 0;			/* mean absolute deviation of RR from rrmean */
    rrinc = 0;			/* maximum incremental change in rrmean */
    rrmean = 0;			/* mean RR interval, in sample intervals */
    rrmax = 0;			/* maximum likely RR interval */
    rrmin = 0;			/* minimum RR interval, in sample intervals */
    rtmax = 0;			/* maximum RT interval, in sample intervals */
    rtmin = 0;			/* minimum RT interval, in sample intervals */
    rtmean = 0;			/* mean RT interval, in sample intervals */
    tamean = 0;			/* mean T-wave amplitude in qfv */
    thresh = 1.0;		/* normalized detection threshold */
    v1 = 0;			/* integral of dv in qf() */
    v1norm = 0;			/* normalization for v1 */
    memset(&annot, 0, sizeof(WFDB_Annotation));		/* most recently written annotation */
    v = NULL;			/* latest sample of each input signal */
    si = NULL;		/* characteristics of each input signal */
    sig = 0;		/* signal number of signal to be analyzed */
    next_minute = 0;		/* sample number of start of next minute */
    spm = 0;			/* sample intervals per minute */
    sps = 0;			/* sample intervals per second */
    t = 0;			/* time of the current sample being analyzed */
    t0 = 0;			/* time of the start of the analysis period */
    tf = 0;			/* time of the end of the analysis period */
    tf_learn = 0;		/* time of the end of the learning period */
    ...
    dt = 0, dt2 = 0, dt3 = 0, dt4 = 0, smdt = 0;	/* time intervals set by gqrs_init() depending
				   on the sampling frequency and used by
				   filter functions sm() and qf();  units
				   are sample intervals */
    smt = 0, smt0;	/* current and starting time for sm() */
    qfv = NULL, smv = NULL;		/* filter buffers allocated by gqrs_init() */
...
Java_ca_uvic_ece_ecg_ECG_GqrsProcess_calBPM( JNIEnv *env, jclass /* this */) {
    init();
    chdir("/sdcard/Heart Carer Data/Temp");
